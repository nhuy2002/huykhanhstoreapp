...
<div class="kh-container">
    <div class="ck-page-header">
        <a href="{{ route('orders.create') }}" class="kh-btn-icon" title="Quay lại chọn món">
    ...
    <form action="{{ route('orders.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="kh-checkout-wrapper">
            ...
            <div class="kh-checkout-left">
                ...
                <div class="kh-card">
                    ...
                    <div id="qr-container" class="kh-qr-box" style="display: none;">
                        ...
                        <img id="vietqr-img" src="{{ $qrUrl ?? '' }}" style="width:200px; height:200px; object-fit:contain; background:white;">
                        <div id="qr-note" style="margin-top:10px; font-size:0.9rem; color:#166534; font-weight: 600;">Nội dung: {{ $qrContent ?? '...' }}</div>
            ...
            <div class="kh-checkout-right">
                <div class="kh-summary-header">Đơn hàng ({{ count($cartItems) }} món)</div>
                <div class="kh-summary-body">
                    @foreach($cartItems as $index => $item)
                        <div class="kh-item-row" onclick="openProductModal({{ $item['id'] }})" style="cursor: pointer;">
                            <img src="{{ $item['image'] ? asset('storage/' . $item['image']) : 'https://via.placeholder.com/50' }}" class="kh-item-img">
                            <div class="kh-item-info">
                                <div class="kh-item-name">{{ $item['name'] }}</div>
                                <div class="kh-item-meta">SL: {{ $item['quantity'] }}</div>
                            </div>
                            <div class="kh-item-price">{{ number_format($item['subtotal'], 0, ',', '.') }} đ</div>
                ...
                <div class="kh-summary-footer">
                    <div class="kh-total-row">
                        ...
                        <span class="kh-total-value">{{ number_format($totalAmount, 0, ',', '.') }} đ</span>
                    </div>
                    <div class="kh-buttons-row">
                        <a href="{{ route('orders.create') }}" class="kh-btn-back">Trở về</a>
...
<script>
    ...
    function openProductModal(id) {
        ...
        if(!data) return;
        document.getElementById('modal-img').src = data.image ? "{{ asset('storage/') }}/" + data.image : 'https://via.placeholder.com/400x300';
