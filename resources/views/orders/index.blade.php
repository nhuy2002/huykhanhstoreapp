...
<div class="kh-container" style="max-width: 1200px; margin: 0 auto; padding: 20px;">
    ...
    <div class="kh-page-header">
        ...
        <form action="{{ route('orders.index') }}" method="GET" class="kh-search-form">
        ...
        <a href="{{ route('orders.create') }}" class="kh-btn-icon primary" title="Tạo đơn mới">
        ...
        <table class="kh-table">
            ...
            <tbody>
                ...
                <tr onclick="openOrderModal({{ $order->id }})" style="cursor: pointer;">
                    <td class="kh-table-code">{{ $order->code }}</td>
                    <td>
                        <div style="font-weight: 500;">{{ $order->customer_name }}</div>
                        <div style="font-size: 0.8rem; color: #64748b;">{{ $order->customer_phone }}</div>
                    </td>
                    <td class="kh-table-money">{{ number_format($order->total_amount, 0, ',', '.') }} đ</td>
                    <td>
                        @php
                        ...
                        @endphp
                        <span class="kh-badge {{ $statusClass }}">{{ $statusText }}</span>
                    </td>
                    ...
                    <td style="text-align: right;">
                        <div style="display: flex; gap: 5px; justify-content: flex-end;">
                            <a href="{{ route('orders.edit', $order->id) }}" class="kh-btn-icon" title="Chỉnh sửa" style="color: #f59e0b;">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            </a>
                            <button class="kh-btn-icon" onclick="openOrderModal({{ $order->id }})" title="Chi tiết">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                            </button>
                            <button class="kh-btn-icon" onclick="confirmDelete({{ $order->id }}, '{{ $order->code }}')" title="Xóa" style="color: #ef4444;">
    ...
    <div class="kh-mobile-list">
        ...
        <div class="kh-order-mobile-card">
            <div class="kh-ord-mob-header">
                <span class="kh-ord-mob-code">{{ $order->code }}</span>
                <span class="kh-ord-mob-date">{{ $order->created_at->format('d/m/Y H:i') }}</span>
            </div>

            <div class="kh-ord-mob-row">
                <span class="kh-ord-mob-label">Khách hàng:</span>
                <span class="kh-ord-mob-val">{{ $order->customer_name }}</span>
            </div>

            <div class="kh-ord-mob-row">
                <span class="kh-ord-mob-label">Tổng tiền:</span>
                <span class="kh-ord-mob-val" style="color: #ef4444;">{{ number_format($order->total_amount, 0, ',', '.') }} đ</span>
            </div>
            ...
            <div class="kh-ord-mob-actions" style="margin-top: 15px; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                <button class="kh-btn-mobile" onclick="openOrderModal({{ $order->id }})" style="background: #3b82f6; color: white; padding: 8px 12px; border-radius: 6px; border: none; font-size: 12px; font-weight: 500; display: flex; align-items: center; gap: 5px;">
                ...
                <a href="{{ route('orders.edit', $order->id) }}" class="kh-btn-mobile" style="background: #f59e0b; color: white; padding: 8px 12px; border-radius: 6px; border: none; font-size: 12px; font-weight: 500; display: flex; align-items: center; gap: 5px; text-decoration: none;">
                ...
                <button class="kh-btn-mobile" onclick="confirmDelete({{ $order->id }}, '{{ $order->code }}')" style="background: #ef4444; color: white; padding: 8px 12px; border-radius: 6px; border: none; font-size: 12px; font-weight: 500; display: flex; align-items: center; gap: 5px;">
...
<script>
    ...
    function openOrderModal(id) {
        ...
        const storageBase = "{{ asset('storage/') }}"; // Lấy URL gốc storage
