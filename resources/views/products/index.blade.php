...
<div class="kh-container" style="max-width: 1200px; margin: 0 auto; padding: 20px;">
    ...
    <form action="{{ route('products.index') }}" method="GET" class="kh-search-form">
        <input type="text" 
               name="search" 
               class="kh-search-input" 
               placeholder="Tìm kiếm theo tên sản phẩm..." 
               value="{{ request('search') }}"> <button type="submit" class="kh-search-btn">
    ...
    <a href="{{ route('products.create') }}" class="kh-btn-icon primary" title="Thêm sản phẩm mới">
    ...
    <tr class="product-row" onclick="openProductModal({{ $product->id }})">
        <td>
            @if($product->image)
                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="kh-product-thumb">
            @else
        ...
        <td>
            <div>Kho: <strong>{{ $product->quantity }}</strong></div>
            <small>Bán: {{ $product->sold }}</small>
        </td>
        <td>
        ...
        <td style="text-align: right;" onclick="event.stopPropagation()">
            <div class="kh-action-group">
                <a href="{{ route('products.edit', $product->id) }}" class="kh-action-btn kh-btn-edit">Sửa</a>
                
                <form action="{{ route('products.destroy', $product->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="button" class="kh-action-btn kh-btn-delete" onclick="deleteProduct(event, {{ $product->id }})">Xóa</button>
                </form>
            </div>
        </td>
    </tr>
...
<div class="kh-mobile-list">
    @foreach($products as $product)
        <div class="kh-product-mobile-card" onclick="openProductModal({{ $product->id }})">
            ...
            {{-- Thông tin sản phẩm (Đã thay bằng dữ liệu động) --}}
            <div class="kh-mob-info">
                <span class="kh-mob-name">{{ $product->name }}</span>
                ...
                <div class="kh-mob-meta">
                    Kho: {{ $product->quantity }} | Bán: {{ $product->sold }}
                </div>
...
<script>
    ...
    function openProductModal(id) {
        ...
        // Link nút sửa
        const editUrl = "{{ route('products.edit', ':id') }}";
